...
<div class="vrcstepsbarcont">
	<ol class="vrc-stepbar" data-vrcsteps="4">
		...
		<li class="vrc-step vrc-step-complete"><a href="<?php echo JRoute::_('index.php?option=com_vikrentcar&task=search&place='.$place.'&pickupdate='.urlencode(date($df, $pickup)).'&pickuph='.date('H', $pickup).'&pickupm='.date('i', $pickup).'&releasedate='.urlencode(date($df, $release)).'&releaseh='.date('H', $release).'&releasem='.date('i', $release).'&returnplace='.$preturnplace.(!empty($pcategories) && $pcategories != 'all' ? '&categories='.$pcategories : '').(!empty($pitemid) ? '&Itemid='.$pitemid : ''), false); ?>"><span class="vrc-step-lbl"><?php echo JText::_('VRSTEPCARSELECTION'); ?></span></a></li>
		<li class="vrc-step vrc-step-current"><span><span class="vrc-step-lbl"><?php echo JText::_('VRSTEPOPTIONS'); ?></span></span></li>
...
<form action="<?php echo JRoute::_('index.php?option=com_vikrentcar&view=vikrentcar'.(!empty($pitemid) ? '&Itemid='.$pitemid : '')); ?>" method="post" class="vrc-showprc-form">
	...
	<div class="vrc-showprc-prices-wrap">
		<h4 class="vrc-showprc-title extra-14-fonts"><?php echo JText::_('VRPRICE'); ?></h4>
				...
				<div class="vrc-showprc-price-row-cell-first">
					<label for="pid<?php echo $t['idprice']; ?>"<?php echo $has_promotion === true ? ' class="vrc-label-promo-price"' : ''; ?>>
						<div class="vrc-showprc-priceinfo">
							<span class="vrc-showprc-pricename extra-14-fonts"><?php echo $has_promotion === true ? '<i class="' . VikRentCarIcons::i('certificate', 'vrc-promo-price-icon') . '"></i> ' : ''; ?><?php echo VikRentCar::getPriceName($t['idprice'], $vrc_tn); ?></span>
							<span class="vrc-showprc-pricecost">
								<?php echo "<span class=\"vrc_price\">".($tax_summary ? VikRentCar::numberFormat($t['cost']) : VikRentCar::numberFormat(VikRentCar::sayCostPlusIva($t['cost'], $t['idprice'])))."</span> <span class=\"vrc_currency\">EUR</span> "; ?>
							</span>
						</div>
					<?php
...
?>
	...
	<div class="car_buttons_box">
		...
		<div class="goback">
			<a class="btn btn-grey" href="<?php echo JRoute::_($backto); ?>"><?php echo JText::_('VRBACK'); ?></a>
		</div>
	</div>
