...
<div class="vrc-cdetails-cinfo">
	...
	<div class="vrc-cdetails-infocar">
		...
		<?php 
		$pozitia=strpos($car['info'],"###");
		?>
		<?php echo do_shortcode(wpautop(substr($car['info'],0,$pozitia))); ?>
		</div>
		...
		if (isset($car_params['reqinfo']) && (bool)$car_params['reqinfo']) {
			...
			?>
			<!-- fisierul este /public_html/wp-content/uploads/vikrentcar/overrides/site/cardetails -->
		<div class="vrc-reqinfo-cont" style="margin-top: 15px;">
...
?>		<div class="vrc-cdetails-cardesc">		<?php		/**		 * @wponly 	we try to parse any shortcode inside the HTML description of the car		 */		echo do_shortcode(wpautop(substr($car['info'],$pozitia+3)));		?>		</div>	<?php	
if ($numcalendars > 0) {
?>
...
if (VikRentCar::allowRent()) {
	...
	$coordsplaces = array();
	$selform = "<div class=\"vrcdivsearch\"><div class=\"vrcdivsearch-inner\"><form action=\"".JRoute::_('index.php?option=com_vikrentcar'.(!empty($pitemid) ? '&Itemid='.$pitemid : ''))."\" method=\"post\" onsubmit=\"return vrcValidateSearch();\">\n";
...
jQuery(function() {
	...
	jQuery('.vr-cal-img, .vrc-caltrigger').click(function() {
		...
		if (!strlen($forced_pickup)) {
			...
			$selform .= '		<label>' . JText::_('VRALLE') . '</label>' . "\n";
		...
		if (!strlen($forced_dropoff)) {
			$selform .= '	<div class="vrcsfentrytime">' . "\n";
			$selform .= '		<label>' . JText::_('VRALLEDROP') . '</label>' . "\n";
			$selform .= '		<div class="vrc-sf-time-container">' . "\n";
	...
	//
	$selform .= "<div class=\"vrcsfentrycont\"><div class=\"vrcsfentrysubmit\"><input type=\"submit\" name=\"search\" value=\"" . JText::_('VRCBOOKTHISCAR') . "\" class=\"btn\"/></div></div>\n";
	// close submit part
	$selform .= "</div>\n";
