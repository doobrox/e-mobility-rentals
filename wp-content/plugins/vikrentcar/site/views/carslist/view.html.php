...
class VikrentcarViewCarslist extends JViewVikRentCar {
	function display($tpl = null) {
		...
		if (is_array($category)) {
			$q = "SELECT `id`,`name`,`img`,`idcat`,`idcarat`,`info`,`startfrom`,`short_info`,`car_order_sorts` FROM `#__vikrentcar_cars` WHERE `avail`='1' AND (`idcat`='".$category['id'].";' OR `idcat` LIKE '".$category['id'].";%' OR `idcat` LIKE '%;".$category['id'].";%' OR `idcat` LIKE '%;".$category['id'].";');";
		} else {
			$q = "SELECT `id`,`name`,`img`,`idcat`,`idcarat`,`info`,`startfrom`,`short_info`,`car_order_sorts` FROM `#__vikrentcar_cars` WHERE `avail`='1';";
		}
