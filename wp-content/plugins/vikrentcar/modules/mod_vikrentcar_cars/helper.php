...
class Modvikrentcar_carsHelper
{
	public static function getCars($params)
	{
		...
		if ($query == 'price') {
			...
			//order by name
			// $q = "SELECT `id`,`name`,`img`,`idcat`,`startfrom`,`short_info`,`idcarat`, `car_order_sorts` FROM `#__vikrentcar_cars` WHERE `avail`='1' ORDER BY `#__vikrentcar_cars`.`car_order_sorts` ".strtoupper($params->get('order'))." LIMIT ".$params->get('numb').";";
			$q = "SELECT `id`,`name`,`img`,`idcat`,`startfrom`,`short_info`,`idcarat`, `car_order_sorts` FROM `#__vikrentcar_cars` WHERE `avail`='1' ORDER BY `#__vikrentcar_cars`.`car_order_sorts` LIMIT ".$params->get('numb').";";
			$dbo->setQuery($q);
			$dbo->execute();
			if ($dbo->getNumRows() > 0) {
				$cars=$dbo->loadAssocList();
